...
const PetTable = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://mapache-server-bio4.onrender.com/get_passwords", {
        headers: { userId },
      })
...
const handleAddPet = async () => {
   ...
   try{
    const response = await fetch('https://mapache-server-bio4.onrender.com/post_pets', {
      method: 'POST', 
      headers: {
    ...
    const data = await response.json();
     ...
     await fetch('https://mapache-server-bio4.onrender.com/sendNotification', {
      method: 'POST', 
      headers: {
        'Content-Type': 'application/json',
      }, 
